...
const TrustedBy = () => {
  return (
    <section className="py-6 bg-white relative overflow-hidden border-b border-black/5">
      <div className="max-w-7xl mx-auto px-6 mb-8 text-center">
        <h3 className="text-sm font-semibold text-primary-dark-blue uppercase tracking-widest">
      ...
      <div className="relative w-full flex overflow-hidden">
        <div className="animate-marquee flex space-x-8 px-4 items-center whitespace-nowrap">
          ...
          {[...sectors, ...sectors].map((sector, idx) => (
            <div key={idx} className="flex items-center space-x-3 px-6 py-3 bg-gray-50 border border-gray-100 rounded-full shadow-sm min-w-max">
              <span className="w-2 h-2 rounded-full bg-royal-blue" />
              <span className="text-sm font-medium text-primary-dark-blue">{sector}</span>
            </div>
          ))}
        </div>
      </div>
